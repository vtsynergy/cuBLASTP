...
struct dpResults gappedExtension_dpBeforeSeed_multi(struct PSSMatrix PSSMatrix, int4 dropoff,
       struct coordinate seed, struct unpackRegion* unpackRegion, int tid);
struct dpResults gappedExtension_dpAfterSeed_multi(struct PSSMatrix PSSMatrix, int4 dropoff,
       struct unpackRegion* unpackRegion, int4 subjectLength,
       int4 seedSubjectOffset, int tid);
struct trace gappedExtension_traceBeforeSeed_multi(struct dpResults beforeDpResults, struct coordinate seed, int tid);
struct trace gappedExtension_traceAfterSeed_multi(struct dpResults beforeDpResults, int4 queryLength, int tid);
...
struct gappedExtension* gappedExtension_build_multi(struct ungappedExtension* ungappedExtension,
                        ...
                        struct unpackRegion* unpackRegion, int4 dropoff, int tid)
{
    ...
    beforeDpResults = gappedExtension_dpBeforeSeed_multi(PSSMatrix, dropoff, seed, unpackRegion, tid);

	// Trace back and create the trace which specifies the first half of the alignment
	beforeTrace = gappedExtension_traceBeforeSeed_multi(beforeDpResults, seed, tid);
	...
	afterDpResults = gappedExtension_dpAfterSeed_multi(choppedPSSMatrix, dropoff, unpackRegion,
                                                 choppedSubjectSize, seed.subjectOffset, tid);

	// Trace back to get the trace for the seed onwards
	afterTrace = gappedExtension_traceAfterSeed_multi(afterDpResults, choppedPSSMatrix.length, tid);
...
int4* gappedExtension_matchRow_t[MAX_NUM_THREADS] = {NULL};
int4* gappedExtension_insertQrow_t[MAX_NUM_THREADS] = {NULL};
int4* gappedExtension_insertSrow_t[MAX_NUM_THREADS] = {NULL};
int4 gappedExtension_rowSizes_t[MAX_NUM_THREADS] = {0};

unsigned char **gappedExtension_traceback_t[MAX_NUM_THREADS] = {NULL};
unsigned char *gappedExtension_tracebackData_t[MAX_NUM_THREADS] = {NULL};
int4 gappedExtension_numRows_t[MAX_NUM_THREADS] = {0};
int4 gappedExtension_tracebackAlloc_t[MAX_NUM_THREADS] = {0};
       ...
       struct coordinate seed, struct unpackRegion* unpackRegion, int tid)
{
	...
	if (gappedExtension_matchRow_t[tid] == NULL)
    {
        // Declare processing rows for storing match, insert-subject and insert-query values
		gappedExtension_rowSizes_t[tid] = 2 * statistics_gappedFinalNominalDropoff / parameters_extendGap;

		// Malloc new rows
		gappedExtension_matchRow_t[tid] = (int4*)global_malloc(sizeof(int4) * gappedExtension_rowSizes_t[tid]);
		gappedExtension_insertQrow_t[tid] = (int4*)global_malloc(sizeof(int4) * gappedExtension_rowSizes_t[tid]);
		gappedExtension_insertSrow_t[tid] = (int4*)global_malloc(sizeof(int4) * gappedExtension_rowSizes_t[tid]);
    }
    ...
    // If more rows are required
    if (seed.queryOffset > gappedExtension_numRows_t[tid])
    {
        // Increase number of row pointers
    	gappedExtension_numRows_t[tid] = seed.queryOffset;
        gappedExtension_traceback_t[tid] = (unsigned char**)global_realloc(gappedExtension_traceback_t[tid],
                                    sizeof(unsigned char*) * (gappedExtension_numRows_t[tid]));
    }

    // If first time, initialize traceback rows
    if (gappedExtension_tracebackData_t[tid] == NULL)
    {
    	gappedExtension_tracebackAlloc_t[tid] = constants_initialTracebackAlloc;
		gappedExtension_tracebackData_t[tid] = global_malloc(sizeof(char) * gappedExtension_tracebackAlloc_t[tid]);
    }

    tracebackDataPtr = gappedExtension_tracebackData_t[tid] + gappedExtension_tracebackAlloc_t[tid] - 1;
	...
	// Initialize row pointers
	rowOffset = (subjectPosition - subject);
	matchRow = gappedExtension_matchRow_t[tid] + (rowOffset % gappedExtension_rowSizes_t[tid]);
	insertQrow = gappedExtension_insertQrow_t[tid] + (rowOffset % gappedExtension_rowSizes_t[tid]);
	insertSrow = gappedExtension_insertSrow_t[tid] + (rowOffset % gappedExtension_rowSizes_t[tid]);
    ...
    // Initialize traceback pointer for this row
	tracebackRow = gappedExtension_traceback_t[tid] + (queryPosition - PSSMatrix.matrix);
	*tracebackRow = tracebackDataPtr - (subjectPosition - subject);
    tracebackColumn = tracebackDataPtr;
    tracebackDataPtr -= (subjectPosition - minRowDropoff + 1);
	...
	*tracebackColumn = 0;
    ...
    // Check for scoring row wrap-around
    if (matchRow < gappedExtension_matchRow_t[tid])
    {
		matchRow += gappedExtension_rowSizes_t[tid];
		insertSrow += gappedExtension_rowSizes_t[tid];
		insertQrow += gappedExtension_rowSizes_t[tid];
    }
...
//	print(gappedExtension_matchRow_t[tid], subject, rowDropoff, columnDropoff);
	...
	while (queryPosition > PSSMatrix.matrix && rowDropoff < columnDropoff)
	{
        ...
        // If not enough space in traceback data for this row, realloc
        if (subjectPosition - minRowDropoff >= tracebackDataPtr - gappedExtension_tracebackData_t[tid])
        {
			newTracebackData = (unsigned char*)global_malloc(sizeof(char) * gappedExtension_tracebackAlloc_t[tid] * 2);

            // Move existing data to end of new block
            memcpy(newTracebackData + gappedExtension_tracebackAlloc_t[tid],
                   gappedExtension_tracebackData_t[tid], gappedExtension_tracebackAlloc_t[tid]);

            // Update traceback data pointer
			tracebackDataPtr = newTracebackData + gappedExtension_tracebackAlloc_t[tid]
                              + (tracebackDataPtr - gappedExtension_tracebackData_t[tid]);

            // Update existing rows to point to new data block
			while (tracebackRow < gappedExtension_traceback_t[tid] + seed.queryOffset - 1)
            {
				tracebackRow++;
            	*tracebackRow = newTracebackData + gappedExtension_tracebackAlloc_t[tid]
                              + (*tracebackRow - gappedExtension_tracebackData_t[tid]);
            }

            // Data block is now double the size
            gappedExtension_tracebackAlloc_t[tid] *= 2;

            free(gappedExtension_tracebackData_t[tid]);
            gappedExtension_tracebackData_t[tid] = newTracebackData;
        }
...
//		print(gappedExtension_matchRow_t[tid], subject, rowDropoff, columnDropoff);
	...
	dpResults.best.subjectOffset = bestSubjectPosition - subject;
	dpResults.bestScore = bestScore;
	dpResults.traceback = gappedExtension_traceback_t[tid];
...
// the given seed
struct dpResults gappedExtension_dpAfterSeed_multi(struct PSSMatrix PSSMatrix, int4 dropoff,
       struct unpackRegion* unpackRegion, int4 subjectLength, int4 seedSubjectOffset, int tid)
{
	int2 **queryPosition, **bestQueryPosition, **queryEnd;
	int2 *matrixColumn;
	unsigned char *subject, *rowDropoff, *columnDropoff, *maxRowDropoff, *newSubject;
	...
	subjectEnd = subject + subjectLength;
	queryEnd = PSSMatrix.matrix + queryLength;
    endMatchRow = gappedExtension_matchRow_t[tid] + gappedExtension_rowSizes_t[tid];

	// Determine lowest score before dropoff
	dropoffThreshold = -dropoff;

    // If more rows are required
    if (queryLength > gappedExtension_numRows_t[tid])
    {
        // Increase number of row pointers
    	gappedExtension_numRows_t[tid] = queryLength;
        gappedExtension_traceback_t[tid] = (unsigned char**)global_realloc(gappedExtension_traceback_t[tid],
                                    sizeof(unsigned char*) * (gappedExtension_numRows_t[tid]));
    }
	...
	// Initialize rows
	matchRow = gappedExtension_matchRow_t[tid] + 1;
	insertQrow = gappedExtension_insertQrow_t[tid] + 1;
	insertSrow = gappedExtension_insertSrow_t[tid] + 1;
...
//    if (dloc==88197331)
//    print2(gappedExtension_matchRow_t[tid], subject, rowDropoff, columnDropoff);
	...
	while (queryPosition < queryEnd && rowDropoff > columnDropoff)
	{
        ...
        // If not enough space in traceback data for this row, realloc
        if (maxRowDropoff - subjectPosition >= gappedExtension_tracebackAlloc_t[tid] -
            (tracebackDataPtr - gappedExtension_tracebackData_t[tid]))
        {
			newTracebackData = (unsigned char*)global_malloc(sizeof(char) * gappedExtension_tracebackAlloc_t[tid] * 2);

            // Move existing data to end of new block
            memcpy(newTracebackData, gappedExtension_tracebackData_t[tid], gappedExtension_tracebackAlloc_t[tid]);

            // Update traceback data pointer
			tracebackDataPtr = newTracebackData + (tracebackDataPtr - gappedExtension_tracebackData_t[tid]);

            // Update existing rows to point to new data block
			while (tracebackRow > gappedExtension_traceback_t[tid])
            {
				tracebackRow--;
            	*tracebackRow = newTracebackData + (*tracebackRow - gappedExtension_tracebackData_t[tid]);
            }
...
//        if (dloc==88197331)
//			gappedExtension_printAfterRow(gappedExtension_matchRow_t[tid], subject, rowDropoff,
//                                          columnDropoff);
	}
...
void gappedExtension_free_multi(int n_threads)
{
    int tid;
    for(tid = 0; tid < n_threads; tid++)
    {
	// Free memory used by traceback array
    free(gappedExtension_traceback_t[tid]);
    free(gappedExtension_tracebackData_t[tid]);

//    printf("gappedExtension_tracebackAlloc=%d\n", gappedExtension_tracebackAlloc);

    // Free memory used to store row scores
    free(gappedExtension_matchRow_t[tid]);
    free(gappedExtension_insertQrow_t[tid]);
    free(gappedExtension_insertSrow_t[tid]);
    }
}
